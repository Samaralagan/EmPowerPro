...
const Events = ({ id }) => {
  ...
  const currentVacancies = vacancies.slice(indexOfFirstCard, indexOfLastCard);
      ...
      <Container>
        <Row className="pt-5">
          <Col fluid className="title-fade text-center" ref={Ref}>
            ...
            <p className="about-text pt-4">{EVENT.text}</p>
          </Col>
        </Row>
        <Row className="d-flex justify-content-center mt-3">
          <Col
            md={3}
            className={`event-top-tap  border-start-round ${
              isActivity ? `event-top-tap-active` : `event-top-tap-nonactive`
            }`}
          ...
          <Col
            md={3}
            className={`event-top-tap  border-end-round ${
              isVacancy ? `event-top-tap-active` : `event-top-tap-nonactive`
            }`}
          ...
          <div className="row mt-4 ps-5 pe-5">
            {currentVacancies.map((vacancy, index) => (
              <div key={index} className="event-vacancy-card">
                <div className="event-vacancy-type">
                  <span>{vacancy.employmentType}</span>
                </div>
                <div
                  dangerouslySetInnerHTML={{ __html: vacancy.jobTitle }}
                  className="event-vacancy-name"
                ></div>
                <div className="event-vacancy-time">
                  {
                    <EVENT_VACANCY_CARD_ICONS.time className="event-vacancy-card-icon" />
                  }
                  {new Date(vacancy.applicationDeadline).toLocaleDateString()}
                  {" (Deadline)"}
                </div>
                <div className="event-vacancy-salary">
                  {
                    <EVENT_VACANCY_CARD_ICONS.salary className="event-vacancy-card-icon" />
                  }{" "}
                  {vacancy.minSalary} - {vacancy.maxSalary}
                </div>
                <div>
                  <button
                    className="event-vacancy-card-button"
                    onClick={() => openModal(vacancy.id)}
                  >
                    READ MORE
                  </button>
                </div>
                <div className="event-vacancy-updated-time">
                  {"Created At "}
            ...
            <Paginator
              currentPage={currentPage}
              totalPages={calculateTotalPages(vacancies, CardsPerPage, Cards)}
              onPageChange={handlePaginationClick}
            />
          </div>
        )}
