...
import { ProjectDone, ProjectInProgress, ProjectToDo } from '../constants/temporary';
                ...
                <div>
                    {ProjectToDo.map((card,index)=>(
                        ...
                        <div className='project-card-color-boxs'>
                           {card.green && (<div className='project-card-color-box' style={{backgroundColor:'#16BD59'}}></div>)} 
                           {card.orange && (<div className='project-card-color-box' style={{backgroundColor:'#EE6401'}}></div>)} 
                           {card.blue && (<div className='project-card-color-box' style={{backgroundColor:'#2DA3B3'}}></div>)} 
                        </div>
                        <div className='project-card-content'>
                                <p>{card.title} </p>
                                <input type="checkbox" />
                        </div>
                        <div className='project-card-date'><FaCalendarWeek className='me-2'/>{card.date}</div>
                        </div>
                    ))}
                </div>        
            </div>
            ...
            <div div className='project-box'>
                ...
                <div>
                    {ProjectDone.map((card,index)=>(
                            <div className='project-card'style={{opacity:0.8}} key={index}>
                                <div className='project-card-color-boxs'>
                                {card.green && (<div className='project-card-color-box' style={{backgroundColor:'#16BD59'}}></div>)} 
                                {card.orange && (<div className='project-card-color-box' style={{backgroundColor:'#EE6401'}}></div>)} 
                                {card.blue && (<div className='project-card-color-box' style={{backgroundColor:'#2DA3B3'}}></div>)} 
