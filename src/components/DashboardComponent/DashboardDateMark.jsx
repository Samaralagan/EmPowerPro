...
import {
  createDateMarker,
  deleteCalenderMarker,
  getAllcalendarMarker,
  updateCalenderMarker,
} from "../../service/IncomeExpenseService";
...
const DashboardDateMark = ({ modalIsOpen, closeModal }) => {
  ...
  function updateMarker(data) {
    ...
    console.log(data);
    updateCalenderMarker(data.id, data)
      .then((response) => {
        console.log(response.data);
      })
      .catch((error) => {
    ...
    <div className="addblog-popup-body">
      ...
      >
        <div>
          ...
          <button
            className="gradient-blue-btn mb-3"
            onClick={() => {
          ...
          {!showlist ? (
            <form>
              <div className="d-flex " style={{ justifyContent: "" }}>
                ...
                <div class="mb-3 ms-5" style={{ position: "relative" }}>
                  ...
                  {errors.time && (
                    ...
                    >
                      <MdOutlineError /> {errors.time}
                    </small>
                  )}
                </div>
              </div>
              <div class="" style={{ position: "relative" }}>
                <label for="exampleInputevent" class="form-label">
                  Event
                </label>
                <input
                  type="text"
                  class="form-control"
                  id="exampleInputevent"
              ...
              <center>
                <button
                  type="submit"
                  className="blogcomment-sent mt-4"
                  onClick={savemarker}
                >
                  Submit
                </button>
            ...
            <div>
              {markData.map((data, index) => (
                ...
                >
                  ...
                  <p>
                    <span
                      className="ps-1 pe-1 m-1 fs-6"
                      style={{
                        color: "Green",
                        backgroundColor: "white",
                        border: "solid 1px",
                      }}
                      onClick={() => updateMarker(data)}
                    >
                      <TiTick />
                    </span>
          ...
          <div className="blogcomment-bottom mt-3"></div>
        </div>
      </Modal>
    </div>
  );
